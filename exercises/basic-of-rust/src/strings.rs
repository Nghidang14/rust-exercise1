...
fn is_palindrome(word: &str) -> bool {
    // todo!()
    let mut old_word: &str = word;
    let chars: Vec<char> = old_word.chars().rev().collect();
    let par_str = String::from_iter(chars);
    word.to_lowercase() == par_str.to_lowercase()
}

// Exercise 6
// Count the occurrences of a character in a string
fn count_char_occurrences(string: &str, character: char) -> usize {
    let mut count = 0;
